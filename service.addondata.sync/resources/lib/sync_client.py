        ...
        for root_path, local_folders, local_files in os.walk(root_folder):
            ...
            if is_root_folder:
                if self.selectiveSync:
                    ...
                    Logger.debug("Selective Sync limited add-ons to:\n"
                                 "Local:\n- %s\n"
                                 "Remote:\n- %s", "\n- ".join(local_folders), "\n- ".join(remote_folders.keys()))

                # see what add-ons can be synced
                local_folders[:] = filter(lambda f: settings.can_add_on_sync(f), local_folders)

                for remote_folder in remote_folders.keys():
                    if not settings.can_add_on_sync(remote_folder):
                        remote_folders.pop(remote_folder)

                Logger.debug("Add-ons limited to:\n"
                             "Local:\n- %s\n"
                             "Remote:\n- %s", "\n- ".join(local_folders), "\n- ".join(remote_folders.keys()))
